...
extern size_t z_embedded_asset_count(void);
extern const char *z_embedded_asset_name(size_t index);
extern const unsigned char *z_embedded_asset_data(size_t index, size_t *size);
extern const unsigned char *z_embedded_asset_find(const char *name, size_t *size);

static bool z_asset_valid_utf8(const unsigned char *data, size_t length) {
    size_t i = 0;
    while (i < length) {
        unsigned char c = data[i++];
        if (c < 0x80) { if (c == 0) return false; continue; }
        size_t remaining = 0; uint32_t codepoint = 0;
        if ((c & 0xe0u) == 0xc0u) { remaining = 1; codepoint = c & 0x1fu; if (codepoint < 2) return false; }
        else if ((c & 0xf0u) == 0xe0u) { remaining = 2; codepoint = c & 0x0fu; }
        else if ((c & 0xf8u) == 0xf0u) { remaining = 3; codepoint = c & 0x07u; if (codepoint > 4) return false; }
        else return false;
        if (i + remaining > length) return false;
        for (size_t j = 0; j < remaining; j++) { unsigned char next = data[i++]; if ((next & 0xc0u) != 0x80u) return false; codepoint = (codepoint << 6u) | (next & 0x3fu); }
        if ((remaining == 1 && codepoint < 0x80u) || (remaining == 2 && codepoint < 0x800u) || (remaining == 3 && codepoint < 0x10000u)) return false;
        if (codepoint > 0x10ffffu || (codepoint >= 0xd800u && codepoint <= 0xdfffu)) return false;
    }
    return true;
}

static ZValue z_asset_call_builtin(const char *name, const ZValue *args, size_t argc, bool *handled) {
    *handled = true;
    if (strcmp(name, "assetList") == 0) {
        z_expect_args(name, argc, 0);
        size_t count = z_embedded_asset_count();
        ZValue *items = count == 0 ? NULL : (ZValue *)z_alloc(sizeof(ZValue) * count);
        for (size_t i = 0; i < count; i++) items[i] = z_string(z_embedded_asset_name(i));
        return z_array_from(items, count);
    }
    if (strcmp(name, "assetExists") != 0 && strcmp(name, "assetText") != 0 && strcmp(name, "assetBytes") != 0) { *handled = false; return z_null(); }
    z_expect_args(name, argc, 1);
    const char *asset_name = z_as_cstring(args[0]);
    size_t size = 0;
    const unsigned char *data = z_embedded_asset_find(asset_name, &size);
    if (strcmp(name, "assetExists") == 0) return z_bool(data != NULL);
    if (data == NULL) z_fatal("%s: embedded asset %s was not found", name, asset_name);
    if (strcmp(name, "assetBytes") == 0) { ZValue result = z_bytes(size); if (size) memcpy(result.as.buffer->data, data, size); return result; }
    if (!z_asset_valid_utf8(data, size)) z_fatal("assetText: embedded asset %s is not valid UTF-8 text", asset_name);
    char *text = (char *)z_alloc(size + 1); if (size) memcpy(text, data, size); text[size] = '\0'; return z_string(text);
}
#endif
...
ZValue z_call_builtin(const char *name, const ZValue *args, size_t argc) {
    ...
    if (asset_handled) return asset_result;
#endif
#if defined(ZUMBRA_ENABLE_UI)
    bool ui_handled = false;
