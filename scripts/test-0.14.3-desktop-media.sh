...
echo "Running Zumbra 0.14.3 desktop media regression tests..."
...
[[ "$("$tmp_dir/zumbra" --version)" == "0.14.3" ]]

cat > "$tmp_dir/media.zum" <<'ZUM'
var app << desktopApp({"backend":"headless","name":"Media","version":"0.14.3"});
...
[[ "$(cat "$tmp_dir/data/zumbra-runtime.txt")" == "0.14.3" ]]
[[ "$output" == "$expected" ]] || {
  printf 'unexpected media output:\n%s\n' "$output" >&2
  exit 1
}

echo "Zumbra 0.14.3 desktop media regression tests passed."
