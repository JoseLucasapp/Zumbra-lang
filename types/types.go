...
const (
	...
	NetListener         Kind = "net_listener"
	NetStream           Kind = "net_stream"
	UDPSocket           Kind = "udp_socket"
	HttpApp             Kind = "http_app"
	HttpServer          Kind = "http_server"
	HttpRequest         Kind = "http_request"
	HttpResponse        Kind = "http_response"
	HttpClientResponse  Kind = "http_client_response"
	...
	SQLRows             Kind = "sql_rows"
	SQLParameters       Kind = "sql_parameters"
	PostgresDatabase    Kind = "postgres_database"
	PostgresStatement   Kind = "postgres_statement"
	PostgresTransaction Kind = "postgres_transaction"
	RedisClient         Kind = "redis_client"
	Config              Kind = "config"
	Logger              Kind = "logger"
	MetricsRegistry     Kind = "metrics_registry"
...
func Compatible(expected, actual *Type) bool {
	...
	if expected.Kind == SQLParameters {
		return actual.Kind == Array || actual.Kind == Dict || actual.Kind == SQLParameters
	}
	if actual.Kind == SQLParameters {
		return expected.Kind == Array || expected.Kind == Dict || expected.Kind == SQLParameters
	}
